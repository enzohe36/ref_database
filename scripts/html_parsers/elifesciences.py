...
def remove_banners(html):
    """Apply Phase 2 layout rules for elifesciences.org.

    Step 1: cap body width at 752 px, center, neutralize @media queries
            so the publisher's narrow CSS branch always applies.
    Step 2: remove the Cookiebot consent dialog and its backdrop underlay.
    """
    html = neutralize_media_queries(html)

    # Step 2 — Cookiebot consent dialog + its backdrop.
    html = remove_elements_by_id(
        html, "CybotCookiebotDialog", "CybotCookiebotDialogBodyUnderlay",
    )
...
def _parse_ref_entry(entry_html):
    ...
    if tm:
        ...
        tm2 = re.search(
            r'<(span|div)[^>]*class="?reference__title\b[^"]*"?[^>]*>(.*?)</\1>',
            entry_html, re.DOTALL,
        )
        if tm2:
            title = unescape(strip_tags(tm2.group(2))).strip()
...
def _parse_main_text(html):
    ...
    Body: every article-section before the first id=references section.
    Post-references: only the Acknowledgements / Ethics subsections of the
    'info' section, plus any section whose id matches _SUPP_RE.
    """
    sections = _find_article_sections(html)
    if not sections:
        return ""

    # Index of the first references section.
    ref_idx = next(
        (i for i, (sid, _, _) in enumerate(sections) if sid.lower() == "references"),
        len(sections),
    )

    body_html = ""
    # Body sections — strictly before references. Drop chrome sections that
    # may appear in this range (none currently observed for eLife, but keep
    # the explicit drop_ids list for safety).
    drop_ids = {"metrics"}
    for sid, _, content in sections[:ref_idx]:
        sid_lc = sid.lower()
        if sid_lc in drop_ids:
            continue
        body_html += "\n" + content + "\n"

    # Post-references: only Acknowledgements / Ethics from 'info', and
    # supplementary sections whose id matches _SUPP_RE.
    for sid, _, content in sections[ref_idx + 1:]:
        sid_lc = sid.lower()
        if sid_lc == "info":
            body_html += "\n" + _extract_info_subsections(content) + "\n"
            continue
