"""Science / AAAS (science.org) HTML parser."""
...
def remove_banners(html):
    """Apply Phase 2 layout rules for science.org (Atypon).
    ...
    Step 3: neutralize sticky/fixed chrome — the resizeable-menu nav,
            main-header__secondary publication strip, fixed
            core-collateral figure-viewer shell, and the fixed
            alert-donation banner. Plus the pb-ad sticky header strip
            and the in-article sticky [data-core-nav=header] section
            navigation.
    Step 5: ad blocks — pb-ad header bar, weby-gam GPT slot,
            aside-ads (incl. sticky variant), and adplaceholder
            reservation wrappers.
    Step 8: add an 8 px bottom margin between figure img and
            figcaption so they're not flush against each other
            (Atypon ships them at 0 px gap natively).
    Step 9: expand collapsed in-scope content — figure caption
            collapsible-wrapper (truncated to ~242 px), reference list
            entries hidden via the HTML5 `hidden` attribute (revealed
            natively by the "Show all references" toggle), and any
            extra-author-list spans hidden behind the "+N authors"
            chip. The corresponding toggle buttons are hidden so they
            don't dangle below the now-expanded content.
    """
    ...
    override = (
        ...
        # gutter padding. html stays full-width so its background paints
        # cleanly behind the centered body.
        ...
        ".resizeable-menu__wrapper,"
        ".main-header__secondary,"
        ".alert-donation,"
        ".core-collateral,"
        "[data-core-nav=header],"
        ".pb-ad.position-sticky"
        "{display:none!important;}"
        # Step 5 — ad blocks (pb-ad, weby-gam GPT slot, aside-ads,
        # adplaceholder reservation wrappers).
        ".pb-ad,"
        ".weby-gam,"
        ".aside-ads,"
        ".adplaceholder"
        ...
        # Step 9 — expand collapsed in-scope content.
        # Figure caption truncation (.collapsible-wrapper.collapsed
        # caps the figcaption at 242 px and fades the rest).
        "article figure .collapsible-wrapper,"
        "article figure .collapsible-wrapper.collapsed"
        "{max-height:none!important;height:auto!important;"
        "overflow:visible!important;}"
        "article figure .collapsible-wrapper.collapsed::after"
        "{display:none!important;content:none!important;}"
        "article figure .collapsible-figure-btn__wrapper"
        "{display:none!important;}"
        # Reference-list truncation: hidden references and the toggle
        # button live inside <section id=bibliography>.
        "section#bibliography [role=listitem][hidden]"
        "{display:block!important;}"
        "section#bibliography .truncation-wrapper"
        "{display:none!important;}"
        # Author-list truncation (Atypon hides extra authors behind a
        # +N affordance; force-show the inline span so every author
        # name reads in the contributors line).
        ".contributors .extra-authors,"
        ".contributors [data-hidden-on]"
        "{display:inline!important;}"
        ".contributors .contributors__count,"
...
def _bibliography_slice(html):
    """Return the inner HTML of <section id=bibliography>, or full html on miss.

    Atypon Science papers wrap the primary reference list in
    <section id=bibliography>; the trailing core-collateral duplicates
    (accordion / tooltip clones) live outside it. Bounding reference
    iteration to this slice prevents the last primary reference's
    8000-char fallback window from running across hundreds of KB into
    the duplicate list and harvesting unrelated DOI/Google-Scholar URLs
    from later citations.
    """
    sm = re.search(r'<section\b[^>]*\bid=["\']?bibliography["\']?[^>]*>', html)
    if not sm:
        return html
    pos = sm.end()
    depth = 1
    open_pat = re.compile(r'<section[\s>]', re.IGNORECASE)
    close_pat = re.compile(r'</section\s*>', re.IGNORECASE)
    while depth > 0 and pos < len(html):
        no = open_pat.search(html, pos)
        nc = close_pat.search(html, pos)
        if not nc:
            break
...
def _parse_references(html):
    ...
    refs = []
    seen_numbers = set()
    # Restrict iteration to the bibliography section so the trailing
    # core-collateral duplicate list (rendered outside the bibliography)
    # cannot leak into the slice of the last primary reference.
    scope = _bibliography_slice(html)
    ...
    # clone is visible. The `hidden` attribute on the wrapping
    # `<div role=listitem>` is NOT a clone marker on Science HTML — it
    # is the accordion-collapsed state used for refs beyond the
    # initially-shown 4. Skipping on `hidden` discards refs 5+ (verified
    # across 11 fixtures spanning 1995–2025); rely on `seen_numbers`
    # alone for dedup.
    ...
    for m in re.finditer(
        ...
        before = scope[max(0, m.start() - 200):m.start()]
