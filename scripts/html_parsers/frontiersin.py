...
def remove_banners(html):
    """Apply Phase 2 layout rules for frontiersin.org.

    Step 1: cap body width at 752 px and center; neutralize @media so
            the publisher's narrow-form CSS applies at any viewport.
    Step 2: OneTrust cookie consent banner + dark backdrop wrapper
            (`#onetrust-consent-sdk` carries both the bottom banner and
            the dark filter).
    Step 3: sticky chrome — the top `Ibar__main` site navbar, the
            slide-out `Ibar__menu` panel, the right-edge `InfoDrawer`
            (share/cite/info), and the IDShare `Modal` overlay all
            stay-on-screen while scrolling. Hide via CSS so the parser
            still sees the source bytes.
    Step 9: expand the truncated affiliation list. Frontiers caps the
            list at 2 items and gates the rest with
            `.AffiliationList__item--visibility { visibility:hidden }`
            plus a "See more" button. Force-reveal hidden items and
            hide the toggle button.
    """
    html = neutralize_media_queries(html)

    # Step 2 — cookie banner + backdrop
    html = remove_elements_by_id(html, "onetrust-consent-sdk")

    override = (
        "<style>"
        # Step 1 — lock layout to 752 px wide, white background.
        ...
        # Step 3 — hide sticky chrome (site header / nav, share drawer,
        # share modal). Visibility-only so the parser still sees source.
        ".Ibar,.Ibar__main,.Ibar__menu,"
        ".InfoDrawer,.IDShare__modal,"
        "div.Modal.IDShare__modal"
        "{display:none!important;}"
        # Step 9 — reveal truncated affiliations and hide the toggle.
        # The publisher gates collapsed items via an outer
        # `.AffiliationList--hidden` modifier that sets
        # `height:0;width:0;visibility:hidden` on `.--visibility` items
        # AND pre-shrinks the second item into inline-flex; restore the
        # default block list rendering for every item.
        ".AffiliationList--hidden .AffiliationList__item,"
        ".AffiliationList--hidden .AffiliationList__item--visibility,"
        ".AffiliationList .AffiliationList__item,"
        ".AffiliationList .AffiliationList__item--visibility"
        "{display:list-item!important;visibility:visible!important;"
        "height:auto!important;width:auto!important;opacity:1!important;}"
        ".AffiliationList .AffiliationList__name"
        "{display:block!important;margin-right:0!important;}"
        ".AffiliationList__buttonWrapper,.AffiliationList__button"
        "{display:none!important;}"
        "</style>"
    )
    # Inject before whichever of </head> or <body comes first.
    # Frontiers SingleFile output omits the main document's </head> and
    # ships an iframe with `srcdoc="...</head>..."` AFTER the article
    # body, so a naive `</head>` replace lands inside the iframe.
    head_idx = html.find("</head>")
    body_m = re.search(r"<body\b", html)
    body_idx = body_m.start() if body_m else -1
    if 0 <= head_idx < (body_idx if body_idx != -1 else len(html)):
        html = html[:head_idx] + override + html[head_idx:]
    elif body_idx != -1:
        html = html[:body_idx] + override + html[body_idx:]
    else:
        html = override + html
    return html
...
def _parse_ref_entry(entry_html):
    ...
    if journal and journal in tail_text:
        ...
        if vm:
            ...
            if vm.group(3):
                pages = vm.group(3).replace("\u2013", "-").rstrip(".,")
    else:
