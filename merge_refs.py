...
- Unresolved references are saved to refs_no_pmid.json for manual editing.

--patch copies single-number entries from refs_no_pmid.json into refs.json
as resolved PMIDs, then removes them from refs_no_pmid.json.
...
from get_refs import load_references, save_references
...
def load_no_pmid():
    """Load refs_no_pmid.json, return dict."""
...
def save_no_pmid(data):
    """Save dict to refs_no_pmid.json with each reference on its own line."""
...
def validate_no_pmid():
    """Copy single-number references from refs_no_pmid.json to refs.json,
    then remove them from refs_no_pmid.json."""
...
def main():
    ...
    subprocess.run([sys.executable, os.path.join(BASE_DIR, "get_journals.py")], check=True)
    ...
    for pmid, entry in refs.items():
        ...
        if needs_ref:
            ...
            # Load already-unresolved citations to skip
            no_pmid = load_no_pmid()
            ...
            for citation_string in citations:
                # Skip citations already recorded as unresolved
                ...
                for attempt in range(2):
                    try:
                        result_pmid, _, _ = search_with_retry(
            ...
            # Record unresolved references in refs_no_pmid.json
            if unresolved:
